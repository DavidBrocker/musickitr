...
Everything runs in native R (auth included — JWT signing uses [`openssl`](https://cran.r-project.org/package=openssl), no external binaries or toolchains required).

## Bring your own credentials

Apple Music access requires an active [Apple Developer Program](https://developer.apple.com/programs/) membership. musickitr never stores, transmits, or logs your credentials anywhere but directly to Apple.

1. In [developer.apple.com](https://developer.apple.com) → **Certificates, IDs & Profiles → Keys**, create a key with **MusicKit** enabled and download the `.p8` file (this is a one-time download — save it somewhere durable).
2. Find your **Team ID** under **Account → Membership**.
3. Note the **Key ID** shown on the key's page.
4. Set the following environment variables (e.g. in `.Renviron` or before launching R):
...
Early scaffold — `mk_search()`, `mk_charts()`, `mk_get()`, and the Jaccard utilities are working and tested against the live API. Contributions (additional endpoint wrappers, visualization helpers, etc.) welcome.
